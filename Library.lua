...
-- Função para criar dropdown no estilo Window:DropDown({ ... })
function Window:DropDown(opts)
    opts = opts or {}

    local container = Instance.new("Frame")
    container.Size = UDim2.new(1, 0, 0, 32)
    container.BackgroundTransparency = 1
    container.Parent = self.Content -- ou outro frame da sua janela onde colocar elementos
    ...
    local currentOption = Instance.new("TextButton")
    ...
    currentOption.Parent = container
    ...
    listContainer.Parent = self.ScreenGui or container.Parent -- Ajuste conforme sua hierarquia
    ...
    local function positionList()
        local absPos = currentOption.AbsolutePosition
        local absSize = currentOption.AbsoluteSize
        listContainer.Position = UDim2.new(0, absPos.X, 0, absPos.Y + absSize.Y + 3)
    end
    ...
    local function toggleList()
        ...
        if open then
            positionList()
            TweenService:Create(listContainer, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Size = UDim2.new(0, 130, 0, #options * 30)}):Play()
        else
    ...
    UIS.InputBegan:Connect(function(input)
        if open and input.UserInputType == Enum.UserInputType.MouseButton1 then
            ...
            local optionPos = currentOption.AbsolutePosition
            local optionSize = currentOption.AbsoluteSize
            ...
            if not (inside(mousePos, listPos, listSize) or inside(mousePos, optionPos, optionSize)) then
                open = false
    ...
    for i, option in ipairs(options) do
        ...
        optBtn.Text = option
        optBtn.Font = Enum.Font.LuckiestGuy
        optBtn.TextSize = 17
    ...
    -- Retorna objeto com Get e Set para controle
    ...
    function dropdownObject:Set(newOption)
        for i, option in ipairs(options) do
...
function Window:Slider(opts)
    ...
    local container = Instance.new("Frame")
    container.Size = UDim2.new(1, 0, 0, 50) -- espaço para label + valor
    container.BackgroundTransparency = 1
    container.Parent = self.Content -- ou onde colocar dentro da janela
    ...
    sliderBar.Size = UDim2.new(1, -20, 0, 18)
