...
local function toggleList()  
    ...
    if open then  
        ...
        TweenService:Create(listContainer, TweenInfo.new(0.2), {Size = UDim2.new(0, 130, 0, targetHeight)}):Play()  
    else  
...
UIS.InputBegan:Connect(function(input)  
    if open and input.UserInputType == Enum.UserInputType.MouseButton1 then  
        ...
        if not (inside(mousePos, lp, ls) or inside(mousePos, dp, ds)) then  
            open = false  
            TweenService:Create(listContainer, TweenInfo.new(0.2), {Size = UDim2.new(0, 130, 0, 0)}):Play()  
        end  
    end  
...
createDropdown(Items, Main, ScreenGui, {  
    Text = "Opção",  
...
local function createSlider(Items, opts, updateSize)
local container = Instance.new("Frame")
